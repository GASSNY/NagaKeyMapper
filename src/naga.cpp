...
std::atomic<bool> repeat_active[20];
#include <chrono>

std::chrono::steady_clock::time_point last_press[20];
...
public:
    ...
    if(find_naga_pair(dev_kbd, dev_mouse)) {
        ...
        if(side_btn_fd != -1 && extra_btn_fd != -1) {
            ...
            std::cout << "Reading from (dynamic): "
                      << dev_kbd << " and " << dev_mouse << std::endl;
        }
    }
    ...
    if(side_btn_fd == -1 || extra_btn_fd == -1) {
        ...
        if ((side_btn_fd = open(device.first, O_RDONLY)) != -1 &&
                ...
                std::cout << "Reading from (legacy): "
                          << device.first << " and " << device.second << std::endl;
                break;
            }
        }
    }
  ...
  void run() {
    ...
    // Intentar control exclusivo (si falla, GNOME seguirá viendo el firmware)
  ...
  std::cout << "Button index: " << i
    ...
    // keydown/key up los simplificamos a "key" (pulsa y suelta)
    const string keydownop = "ydotool key ";
    const string keyupop   = "ydotool key ";
    // click sin --delay
    const string clickop   = "ydotool click ";
    // workspace_* no existen realmente en ydotool, pero como tú no los usas,
    // los dejamos como están o los comentamos si quieres evitar errores:
    const string workspace_r = "true ";
    const string workspace   = "true ";
    // mousemove sí existe, este lo dejamos igual
